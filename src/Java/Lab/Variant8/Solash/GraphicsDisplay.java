...
// Перо для рисования графика
        float[] dash = {20,5,10,5,10,5,20,5,5};
...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        double sum = 0;
        int tmp =0;
        for (Double[] i : graphicsData) {
         //   int temp = 0;
            double y = i[1];
                sum +=y;
                tmp++;
        }
        double avarage = sum/tmp;
        ...
        for (Double[] point : graphicsData) {
            ...
            //задание
            Boolean moreThen2Times = null;
            if ( y > 2*avarage) moreThen2Times = true;
            else moreThen2Times = false;
            if(moreThen2Times) canvas.setColor(Color.BLUE);
            else canvas.setColor(Color.RED);


            canvas.drawLine((int) center.getX(), (int) center.getY() - 5, (int) center.getX(), (int) center.getY() + 5);

            canvas.drawLine((int) center.getX()-5, (int) center.getY() - 5, (int) center.getX()+5, (int) center.getY() + 5);
